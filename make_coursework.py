...
def build_docx():
    ...
    add_title_line("на тему: «Разработка базы данных и веб-приложения для автоматизации учета на предприятии ООО «ТехноПромСборка»»", size=15, bold=True, space_after=150)
    ...
    doc.add_paragraph(
        "Актуальность разработки информационной системы для предприятия ООО «ТехноПромСборка» заключается в автоматизации "
    ...
    doc.add_paragraph(
        "Предприятие ООО «ТехноПромСборка» выпускает готовую продукцию из комплектующих деталей. Детали поступают на склад комплектующих, "
    ...
    add_title_line("[Рисунок 1 – ER-диаграмма связей сущностей базы данных ООО «ТехноПромСборка»]", size=12, italic=True, space_before=12, space_after=12)
    ...
    doc.add_paragraph(
        ...
        "Созданная СУБД ООО «ТехноПромСборка» обеспечивает целостность данных за счет внешних ключей и ограничений. "
    ...
    doc.save("Курсовая_работа_ТехноПромСборка.docx")
    print("Файл Курсовая_работа_ТехноПромСборка.docx успешно обновлен по шаблону!")
